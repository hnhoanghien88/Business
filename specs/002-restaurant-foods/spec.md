...
**Feature Branch**: `[002-restaurant-foods]`
...
**Input**: User description: "Cập nhật feature Món ăn & biến thể theo canonical naming Foods, lưu dữ liệu món trong bảng restaurant_foods và chuẩn hóa tích hợp Identity Application Restaurant."
...
4. **Given** màn hình được khởi tạo hoặc hiển thị lại nhiều lượt đồng thời với cùng điều kiện danh sách, **When** dữ liệu chưa tải xong, **Then** hệ thống chỉ thực hiện một lần lấy dữ liệu và chia sẻ cùng kết quả cho các lượt hiển thị đó.
...
3. **Given** người dùng có quyền `Foods.Update`, **When** nhập giá mới hợp lệ, **Then** giá hiện tại thay đổi và lịch sử giá cũ/mới vẫn tra cứu được.
4. **Given** người dùng có quyền `Foods.Update`, **When** quản lý biến thể, giá hoặc availability, **Then** các thao tác cập nhật tương ứng được cho phép mà không cần quyền chuyên biệt.
...
- **FR-010**: Giá MUST không âm và MUST chỉ được thay đổi qua hành động đổi giá dành cho người có quyền `Foods.Update`.
...
- **FR-017**: Menu và permission MUST dùng Resource `Foods`; các permission `Product.*` cũ MUST được migrate hoặc ánh xạ tương thích có thời hạn trong giai đoạn chuyển đổi. Mọi thao tác cập nhật món, biến thể, giá và availability MUST dùng chung quyền `Foods.Update`; hệ thống MUST NOT yêu cầu `Foods.ChangePrice` hoặc `Foods.ChangeAvailability`.
...
- **FR-021**: Menu, route và permission của feature MUST thuộc Identity Application code `Restaurant`; mã ứng dụng và token audience MUST khớp chính xác, bao gồm chữ hoa/chữ thường, giữa Identity, giao diện và dịch vụ nghiệp vụ.
- **FR-022**: Dữ liệu món MUST được lưu trong `restaurant_foods`; dữ liệu biến thể và lịch sử giá MUST lần lượt được lưu trong `restaurant_food_variants` và `restaurant_food_price_histories`, không tạo bảng Product song song.
- **FR-030**: Hệ thống MUST hợp nhất các lượt lấy danh sách món đồng thời có cùng điều kiện thành một yêu cầu đang xử lý; các lượt với điều kiện khác nhau MUST vẫn được xử lý độc lập.
...
- **SC-011**: 100% lượt mở màn hình tạo ra nhiều yêu cầu danh sách món đồng thời với cùng điều kiện chỉ được hệ thống xử lý như một lần lấy dữ liệu, không làm thay đổi kết quả hiển thị hoặc khả năng thử lại.
...
- Foods là tên canonical duy nhất của menu, route và controller; các route Product cũ chỉ được giữ làm chuyển hướng tương thích có thời hạn, không tạo catalog thứ hai.
- Dữ liệu món được lưu trong bảng nghiệp vụ `restaurant_foods`; biến thể và lịch sử giá lần lượt dùng `restaurant_food_variants` và `restaurant_food_price_histories`.
- Giao diện và dịch vụ nghiệp vụ sử dụng Identity Application code `Restaurant`; application code và token audience được xem là phân biệt chữ hoa/chữ thường tại ranh giới xác thực.
